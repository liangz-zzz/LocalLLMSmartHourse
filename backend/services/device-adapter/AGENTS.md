...
- 运行 HA 模式（发现/同步来自 Home Assistant）：`MODE=ha HA_BASE_URL=http://homeassistant:8123 HA_TOKEN=<token> docker compose -f deploy/docker-compose.yml run --rm device-adapter npm run dev`
  - 可选：`HA_INCLUDE_DOMAINS=switch,light,cover,climate` / `HA_EXCLUDE_DOMAINS=...`
