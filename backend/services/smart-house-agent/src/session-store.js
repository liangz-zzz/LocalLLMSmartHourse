...
function normalizeSession({ data, sessionId, maxMessages }) {
  ...
  if (!session.state || typeof session.state !== "object") {
    session.state = { lastDeviceId: null, lastDeviceName: null, lastRoom: null, wakeSource: null, lastExecution: null, pending: null };
  } else {
    ...
    if (session.state.wakeSource === undefined) session.state.wakeSource = null;
