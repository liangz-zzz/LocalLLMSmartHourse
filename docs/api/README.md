...
- REST：见 `openapi.yaml`（设备列表/详情、动作下发与查询、规则 CRUD、户型/布局与资产上传、场景运行 `/scenes/{id}/run`、Agentic 场景 `/scenes/{id}/plan` + `/scenes/{id}/agent-run` + `/scene-runs/{runId}`、模拟设备配置 `/virtual-devices/*`、健康检查，支持 ApiKey/JWT）。
